...
interface Control {
  ...
  description: string;
  danger:      boolean;
  invertLogic?: boolean; // when true, enabled=false is the alarming state
  enabled:     boolean;
}
...
function ControlCard({
  ...
  // Whether the current state is "alarming" (platform under stress)
  const isAlarming = control.invertLogic ? !control.enabled : control.enabled;

  const handleToggleClick = () => {
    // Always require confirmation for emergency toggles
    setConfirming(true);
    setFeedback('idle');
  };
  ...
  const handleConfirm = () => {
    ...
    start(async () => {
      try {
        // All emergency toggles require a fresh TOTP code — they are all nuclear
      ...
      {confirming && (
        <div className="mt-4 pt-4 border-t border-gray-200 space-y-3">
          <div className="p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800 font-medium flex items-center gap-1.5">
            <ShieldAlert size={12} />
            Confirm: This will {control.enabled ? 'DISABLE' : 'ENABLE'} <strong>{control.label}</strong> for all users immediately.
          </div>
...
export function EmergencyClient({ controls, recentActions, onToggle }: EmergencyClientProps) {
  const { requireStepUp, StepUpModal } = useStepUpAuth();

  return (
    <div className="space-y-8">
      {/* Step-up modal — overlays the whole emergency panel */}
      {StepUpModal}

      {/* Controls grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {controls.map((c) => (
          <ControlCard key={c.key} control={c} onToggle={onToggle} requireStepUp={requireStepUp} />
        ))}
      </div>
