...
//      - Bank details updated within N hours → hold 24h (unconditional)
//      The bank-details hold DOES NOT gate on !hasCompletedOrders.
// FIX: Check 2 now uses wallets.bank_details_updated_at (TIMESTAMPTZ, set
//      exclusively by the trigger_bank_details_timestamp BEFORE UPDATE trigger).
// FIX: Added withdrawal_gate_threshold enforcement (Check 0).
// FIX (PARTIAL → COMPLETE — dual-channel notification):
//      Withdrawal-held event now delivers a notifications row AND an F9 inbox
//      message via sendF9SystemMessage() (shared from @/lib/messaging/system-message).
// FIX (PARTIAL → COMPLETE — auto-release):
//      Each hold check now writes hold_release_at into the withdrawals row:
//        Check 0 (gate threshold) → NULL  (admin manual review, no auto-release)
//        Check 1 (wallet funded)  → now + 24h
//        Check 2 (bank details)   → now + 24h
//        Check 3 (trust 40–59)    → now + 48h
...
async function initiateWithdrawal(formData: FormData) {
  ...
  // ── Platform-wide withdrawal pause check ─────────────────────────────────
  const adminClient = createAdminClient();

  const { data: pauseConfig } = await adminClient
    .from('platform_config')
    .select('enabled')
    .eq('key', 'withdrawals_paused')
    .single();

  if (pauseConfig?.enabled === true) {
    redirect('/freelancer/earnings?error=withdrawals_paused');
  }
  ...
  const config = await getPlatformConfigs(adminClient, [
    ...
    CONFIG_KEYS.WITHDRAWAL_GATE_THRESHOLD,
  ]);
  ...
  if (WITHDRAWAL_GATE_THRESHOLD > 0 && amount > WITHDRAWAL_GATE_THRESHOLD) {
    withdrawalStatus = 'held';
    holdReleaseAt    = null;
    holdReason       =
      `This withdrawal (₦${amount.toLocaleString('en-NG')}) exceeds the platform's ` +
      `manual-review threshold of ₦${WITHDRAWAL_GATE_THRESHOLD.toLocaleString('en-NG')}. ` +
  ...
  if (!holdReason && bankDetailsUpdatedAt !== null) {
    if (now - bankDetailsUpdatedAt < BANK_UPDATE_HOLD_MS) {
      withdrawalStatus = 'held';
      holdReleaseAt    = new Date(now + HOLD_24H_MS).toISOString();
      holdReason       =
        `Withdrawal held for 24 hours: bank account details were recently configured ` +
        `within the last ${config[CONFIG_KEYS.BANK_UPDATE_HOLD_HOURS]} hour(s). ` +
        `This is an automated fraud-prevention hold.`;
    }
  }

  // Check 3 — Trust Score tier 40–59 → 48h auto-release.
  if (!holdReason && trustScore >= 40 && trustScore <= 59) {
    withdrawalStatus = 'held';
    holdReleaseAt    = new Date(now + HOLD_48H_MS).toISOString();
    holdReason       = 'Mandatory 48-hour delay applied due to Trust Score tier (40–59).';
  }
  ...
  if (withdrawalStatus === 'held' && holdReason) {
    await Promise.all([
      ...
      sendF9SystemMessage(adminClient, user.id, holdReason),
    ]);
...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  withdrawals_paused: 'Withdrawals are currently paused platform-wide. Please try again later.',
          ...
          <div className="text-center py-6">
            ...
            <div className="mt-4 w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${Math.min((availableBalance / minimumWithdrawal) * 100, 100)}%`,
                }}
              />
            </div>
