...
async function initiateWithdrawal(formData: FormData) {
  ...
  // bank_details_updated_at is set exclusively by trigger_bank_details_timestamp
  // (BEFORE UPDATE on wallets) whenever bank_name, account_number, or account_name
  // changes. It is NULL until bank details are first saved post-migration.
  ...
  //
  // Only used by Check 1. Check 2 (bank details hold) is unconditional per
  // spec — a freelancer's order history does NOT bypass the bank-change hold.
  ...
  // wallets.updated_at is used here as a "recently funded" proxy because
  // process_successful_payment explicitly sets updated_at when crediting
  // pending_clearance. This hold is intentionally scoped to new accounts
  // (hasCompletedOrders === false) — established freelancers are trusted.
  ...
  // Check 2 — bank details updated within configured window.
  // Uses bank_details_updated_at (set by trigger_bank_details_timestamp only
  // when bank_name / account_number / account_name actually change) rather than
  // updated_at (which fires on every wallet mutation including balance credits).
  // This hold is UNCONDITIONAL: it applies regardless of order history because
  // the spec does not define a !hasCompletedOrders bypass for this rule.
