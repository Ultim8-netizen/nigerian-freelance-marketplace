...
//      Spec: "New bank account added → withdrawal requested within 48h → auto-hold 24h"
//      — no order-history bypass is defined for this rule.
// FIX: Check 2 now uses wallets.bank_details_updated_at (TIMESTAMPTZ, set exclusively
//      by the trigger_bank_details_timestamp BEFORE UPDATE trigger) instead of
//      wallets.updated_at (which is mutated by process_successful_payment,
//      release_escrow_to_wallet, and update_wallet_on_withdrawal_complete — i.e. any
//      balance event — making it unsuitable as a "bank details changed" proxy).
// NOTE: Regenerate database.types.ts from Supabase after applying the migration that
//       adds wallets.bank_details_updated_at, so the column is reflected in the
//       autogenerated types and the wallet select is fully typed without a cast.
// UPDATED: Hold thresholds (wallet_fund_hold_hours, bank_update_hold_hours) and the
//          withdrawal gate threshold now read from platform_config via
//          createAdminClient() — RLS on platform_config is admin-only so the regular
//          user client cannot read it. Hardcoded constants removed; defaults preserved
//          as fallback inside getPlatformConfigs if the config rows are absent.
// FIX: Added withdrawal_gate_threshold enforcement (Check 0):
//      When an admin sets a ₦ threshold via the admin finance panel, any withdrawal
//      above that amount is automatically held for manual review, regardless of the
//      freelancer's order history or trust score.
...
async function initiateWithdrawal(formData: FormData) {
  ...
  // withdrawals_paused is toggled by the admin finance panel. It is stored in
  // platform_config with admin-only RLS, so createAdminClient() is required.
  // This check runs first — before wallet, profile, or trust reads — so the
  // gate takes immediate effect with no bypass path.
  ...
  // adminClient is reused from the pause check above.
  ...
  const WITHDRAWAL_GATE_THRESHOLD = config[CONFIG_KEYS.WITHDRAWAL_GATE_THRESHOLD]; // ₦, 0 = disabled
  ...
  //
  // Only used by Check 1. Checks 0, 2, 3 are unconditional per spec.
  ...
  // Check 0 — Admin-configured withdrawal gate threshold.
  // When an admin sets a ₦ threshold in the finance panel (withdrawal_gate_threshold),
  // any withdrawal above that amount is automatically held for manual review.
  // This check runs first and is unconditional: it applies regardless of
  // trust score, order history, or bank-details recency.
  if (WITHDRAWAL_GATE_THRESHOLD > 0 && amount > WITHDRAWAL_GATE_THRESHOLD) {
    withdrawalStatus = 'held';
    holdReason =
      `This withdrawal (₦${amount.toLocaleString('en-NG')}) exceeds the platform's manual-review threshold of ₦${WITHDRAWAL_GATE_THRESHOLD.toLocaleString('en-NG')}. An admin will review and approve it shortly.`;
  }

  // Check 1 — wallet funded within configured window AND zero completed orders.
  ...
  // Check 2 — bank details updated within configured window (unconditional).
  ...
  if (!holdReason && bankDetailsUpdatedAt !== null) {
    if (now - bankDetailsUpdatedAt < BANK_UPDATE_HOLD_MS) {
      withdrawalStatus = 'held';
      holdReason =
        `Withdrawal held for 24 hours: bank account details were recently configured within the last ${config[CONFIG_KEYS.BANK_UPDATE_HOLD_HOURS]} hour(s). This is an automated fraud-prevention hold.`;
    }
  }
